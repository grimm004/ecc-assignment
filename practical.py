...
# Convert a binary vector v to its equivelant in decimal
vectorToDecimal = lambda v: sum(v[i] * (2 ** (len(v) - i - 1)) for i in range(len(v)))
# Convert a binary vector v to a binary vector matrix (which can be multiplied by other matrices)
vectorToMatrix = lambda v: [v]
# Convert a binary vector matrix m to a binary vector
matrixToVector = lambda m: m[0] if len(m) > 0 else []

# Create a H matrix with length (2 ** n) - 1
getH = lambda n: [[(j >> (n - i - 1)) & 1 for j in range(1, 2 ** n)] for i in range(n)]
# Calculate the the n value corresponding to r
getN = lambda r: (2 ** r) - 1
# Calculate the k value corresponding to r
getK = lambda r: getN(r) - r

# Multiply one matrix m0 by another m1
...
# Evaluate whether a codeword length l is valid and, if so, return the corresponding values for r and k
...
# Evaluate whether a hamming code length l is valid and, if so, return the corresponding values for r and n
...
# If this is not being imported, run some test code
if __name__ == '__main__':
    from random import randint
    allPass = True
    for i in range(10000):
        # Access a pseodo-random number to send
        numberToSend = randint(0, (2**32) - 1)
        nBits = 32
        # Encode number to a hamming code
        ...
        # If the sent number is equal to the received number, mark it as a pass
        allPass &= numberToSend == receivedNumber

    print("all passed" if allPass else "fail occurred")
